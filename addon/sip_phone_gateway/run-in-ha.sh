...
MQTT_COMMAND_TOPIC="$(bashio::config 'mqtt.command_topic')"
MQTT_EVENT_TOPIC="$(bashio::config 'mqtt.event_topic')"

if bashio::var.true "${MQTT_ENABLED}"; then
    printf -v MQTT_OPTIONS ' --enable-mqtt --mqtt-address %q --mqtt-port %q --mqtt-username %q --mqtt-password %q --mqtt-topic %q --mqtt-state-topic %q' \
        "${MQTT_HOST}" "${MQTT_PORT}" "${MQTT_USERNAME}" "${MQTT_PASSWORD}" "${MQTT_COMMAND_TOPIC}" "${MQTT_EVENT_TOPIC}"
    export GLOBAL_OPTIONS="${GLOBAL_OPTIONS}${MQTT_OPTIONS}"
fi
